...
Ruby supports multiple types of variables, each with a specific scope and purpose:

### Local Variables
- Defined using normal naming conventions
- Scoped to the method or block where they are declared
- Cannot be accessed outside that scope
...
**Instance variables** are tied to individual objects and are not shared between instances.
- **Class variables** are shared across all instances and subclasses, which can lead to unexpected behavior if not used carefully.
...
- `nil` itself is an object in Ruby

## Symbol
- Immutable and unique
- Commonly used as hash keys
- Faster lookup compared to strings due to immutability
